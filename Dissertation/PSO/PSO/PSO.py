...
c1,c2,nx,ny,w,err = 0.5,0.5,2,2,0.2,0.001
...
# Big loop.
while ((f1darr(gbest-err)<fvalues[step])&(fvalues[step]<(f1darr(gbest+err)))).all()==False:
...
fig = ax.scatter(positions[:,:,0],positions[:,:,1]), plt.title(f'Steps:{step}, Time taken:{np.round(time.time()-start_time,6)}s')
